...
#
# By DEFAULT that tree is the latex/ directory these scripts live in, so the repo
...
#
# The scripts live in latex/ but read their inputs from the repo's results/
# (one level up), so paths are resolved from the script directory, not the cwd.
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))   # the plotting scripts (latex/)
REPO_ROOT = os.path.dirname(_SCRIPT_DIR)                   # code-repo root, holds results/
RESULTS_DIR = os.path.join(REPO_ROOT, "results")
REPO_LATEX_ROOT = _SCRIPT_DIR                              # default output tree = here
...
def emit_fragment(path, body, note=None):
    ...
    if note:
        header += note.rstrip("\n") + "\n"
    header += "% Regenerate with: PAPER_ROOT=<paper> python3 latex/make_<script>.py\n"

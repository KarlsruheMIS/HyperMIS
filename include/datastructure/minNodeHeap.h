...
#include "datastructure/priority_queue_interface.h"
...
class minNodeHeap : public priority_queue_interface {
        public:
                ...
                void insert(NodeID id, NodeID gain); 
                ...
                void decreaseKey(NodeID node, NodeID gain);
                void increaseKey(NodeID node, NodeID gain);
                void changeKey(NodeID node, NodeID gain); 
                NodeID getKey(NodeID node); 
...
inline NodeID minNodeHeap::minValue() {
...
inline void minNodeHeap::insert(NodeID node, NodeID gain) {
...
inline void minNodeHeap::changeKey(NodeID node, NodeID gain) {
        NodeID old_gain = m_heap[m_elements[m_element_index[node]].get_index()].first; 
        if( old_gain > gain ) {
...
inline void minNodeHeap::decreaseKey(NodeID node, NodeID gain) {
...
inline void minNodeHeap::increaseKey(NodeID node, NodeID gain) {
        assert(m_element_index.find(node) != m_element_index.end());
        int queue_idx = m_element_index[node]; 
        int heap_idx  = m_elements[queue_idx].get_index();
        m_elements[queue_idx].set_key(gain);
...
inline NodeID minNodeHeap::getKey(NodeID node) {
